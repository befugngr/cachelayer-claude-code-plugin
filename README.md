...
Windows packaging includes `cachelayer_tools/run_mcp.cmd` and `run_verify_hook.cmd`. Claude plugin hook manifests do not expose a per-OS command selector, so the managed remote MCP remains usable on Windows while local hooks require Git Bash or a user MCP override to the bundled `.cmd` launcher. Failures remain fail-open.
