...
Set `CACHELAYER_KEY` to your `clct_<token>`. Silent hooks handle bounded lookup/save for explicit read/search tools only (no network without a token); do not MCP-tax every step. Set `CACHELAYER_WORKSPACE_ROOT` when Claude is launched outside the target repository.
